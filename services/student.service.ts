...
export interface StreamCertificate {
  ...
  certificateDispatchStatus?: "NOT_DISPATCHED" | "DISPATCHED";
  certificateDispatchedAt?: string | null;
  createdAt?: string;
  student?: { id: number; name: string; rollNo: string } | null;
  stream?: { id: number; name: string } | null;
}

export async function getAdminStreamCertificates(
  params?: Record<string, unknown>,
) {
  const res = await api.get("/admin/certification/stream-certificates", {
    params,
  });
  return normalizePaginatedResult<StreamCertificate>(unwrapData(res));
}

export async function approveStreamCertificate(id: number) {
  const res = await api.patch(
    `/admin/certification/stream-certificate/${id}/approve`,
  );
  return unwrapData(res);
}

export function getAdminStreamCertificatePdfUrl(id: number): string {
  const baseUrl = api.defaults.baseURL || getApiBaseUrl();
  return `${baseUrl}/admin/certification/stream-certificate/${id}/pdf`;
}

export async function getStudentStreamCertificates(studentId: number) {
  const res = await api.get(
    `/certification/stream-certificates/student/${studentId}`,
  );
