...
// Moved VERBATIM from app/ci/agreement/page.tsx (multi-franchise pass): the
// single-agreement experience — stepper shell, SignatureStep, per-agreement
// SIGNED/EXPIRED/VOID views — now parameterized by `agreement` so the page
// can orchestrate one flow per franchise agreement. Signing calls are
// untouched (byte-identical signing constraint).
...
// ─── Per-agreement terminal views ─────────────────────────────────────────────

/**
 * Shown when the CI's agreement has expired (phase EXPIRED). Mirrors the
 * franchisee AgreementExpiredView, but CI renewals are admin-driven (no
 * signing or payment by the CI), so this is purely informational.
 */
export function CIAgreementExpiredView({ agreement }: { agreement: CIAgreementRecord }) {
...
/**
 * Shown when the CI's agreement has been voided (status Void). Void is a
 * deliberate, permanent termination by Abacus — unlike an expiry, it is not
 * renewed automatically, so the CI is directed to contact Abacus.
 */
export function CIAgreementVoidView() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-lg overflow-hidden rounded-2xl border-border bg-card shadow-sm">
        <CardHeader className="border-b bg-accent/30 px-5 py-5 text-left">
...
interface AgreementSignFlowProps {
  agreement: CIAgreementRecord;
  /** Called after any signature write so the caller can refetch + refresh the gate. */
  onSigned: () => Promise<void> | void;
}

export function AgreementSignFlow({ agreement, onSigned }: AgreementSignFlowProps) {
