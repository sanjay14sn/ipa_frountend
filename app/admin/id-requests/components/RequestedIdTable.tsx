...
export default function RequestedIdTable({
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        setLoading(true);
        const result = await getPaginatedRequestedIdDetails({
          page: currentPage,
          limit,
        ...
        console.error("Error fetching requested ID details:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-gray-600">
        {loading ? "Loading..." : `Showing ${students.length} of ${total} requested IDs`}
      </div>
      ...
      {!loading && students.length === 0 && (
        <div className="text-center py-8">
          <div className="text-gray-500">
            No requested IDs found matching your criteria
          </div>
        </div>
      )}
    </div>
