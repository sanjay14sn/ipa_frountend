...
export default function ApprovedCourseInstructorsTable({
  ...
  useEffect(() => {
    const fetchInstructors = async () => {
      try {
        setLoading(true);
        const result = await getPaginatedCourseInstructors("Approved", {
          page: currentPage,
          limit,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-gray-600">
        {loading ? "Loading..." : `Showing ${instructors.length} of ${total} approved instructors`}
      </div>
      ...
      {!loading && instructors.length === 0 && (
        <div className="text-center py-8">
          <div className="text-gray-500">
            No approved instructors found matching your criteria
          </div>
        </div>
      )}
    </div>
