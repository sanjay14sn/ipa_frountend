...
import ProgramRequestsTable from "./components/ProgramRequestsTable";
...
export default function ProgramRequestsPage() {
  ...
  const handleApprove = async (request: ProgramRequestRow) => {
    ...
    try {
      const kitData = await getProgramKits(request.programId);
      setKits(kitData ?? []);
    } catch {
      setKits([]);
    }
  ...
  const selectedKitsCount = kits.filter(
    (k) => selectedKits[k.inventoryId] && selectedKits[k.inventoryId] > 0
  ).length;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Review and manage program requests from franchisees with filtering
                      ...
                      >
                        <div className="p-2">
                          ...
                          <div className="max-h-[300px] overflow-y-auto">
                            {kits.map((kit) => {
                              const isSelected = !!(
                                selectedKits[kit.inventoryId] &&
                                selectedKits[kit.inventoryId] > 0
                              );
                              const qty = selectedKits[kit.inventoryId] || 1;
                              return (
                                <div
                                  key={kit.id}
